...
public class AuthController : ControllerBase
{
    ...
    [HttpPost("register")]
    [AllowAnonymous]
    public ActionResult<UserDto> Register([FromBody] RegisterDto dto, [FromQuery] string otp)
    {
        if (string.IsNullOrWhiteSpace(otp))
            return Unauthorized("OTP required");

        var otpService = HttpContext.RequestServices.GetRequiredService<OtpService>();
        if (!otpService.Validate(dto.UserName, "Register", otp, true))
    ...
    [HttpPost("login")]
    [AllowAnonymous]
    public async Task<ActionResult<UserDto>> Login([FromBody] LoginDto dto, [FromQuery] string otp)
    {
        if (string.IsNullOrWhiteSpace(otp))
            return Unauthorized("OTP required");

        var otpService = HttpContext.RequestServices.GetRequiredService<OtpService>();
        if (!otpService.Validate(dto.UserName, "Login", otp, true))
            return Unauthorized("Invalid or expired OTP");
